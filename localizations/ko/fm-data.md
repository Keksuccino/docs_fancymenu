...
description: FancyMenu를 사용하여 서버와 클라이언트 간에 사용자 지정 데이터를 보내고 받습니다.
---

# FM 데이터

"FM Data" 시스템을 사용하면 서버와 클라이언트 사이에 사용자 지정 텍스트 데이터를 주고받을 수 있습니다.

모든 FM Data 메시지에는 다음이 포함됩니다:
...
3. 클라이언트는 액션 **Send FM Data To Server**로 다시 데이터를 보낼 수도 있습니다.
4. 서버는 `/fmdata listener ...`로 자동 반응할 수 있습니다.
5. 서버는 `/fmdata welcome_data ...`로 접속 시 자동 전송을 설정할 수 있습니다.
...
- `<target_player>`는 `@a`, `@p`, `@s` 같은 일반 플레이어 셀렉터를 지원합니다.
- 공백이 있는 값은 따옴표로 감싸세요.
...
- 멀티플레이어에서는 서버 IP
- 싱글플레이어에서는 `integrated_server`

일반적인 사용 사례:

- 텍스트 요소 업데이트
- 메뉴 액션 트리거
- 들어온 식별자/데이터에 따라 로직 실행
...
이후 서버는 `/fmdata listener ...`로 들어오는 데이터를 처리할 수 있습니다.

# 서버 리스너

서버 리스너는 클라이언트로부터 들어오는 데이터를 감지하며, 트리거되면 하나 또는 여러 개의 명령을 실행할 수 있습니다.
...
`matching_type_identifier`와 `matching_type_data`는 다음이 될 수 있습니다:
...
- `ignore_case_identifier`와 `ignore_case_data`는 true/false 토글입니다
- `listen_for_identifier`는 와일드카드 `*`를 지원합니다(항상 일치)
- `listen_for_data`는 와일드카드 `*`를 지원합니다(항상 일치)
- `fire_for_player`는 일반 플레이어 셀렉터를 사용합니다(예: `@a`, `@p`, `Player761`)
...
- 여러 명령은 `|||`로 구분합니다
- 리터럴 구분자 자체를 넣으려면 `\|\|\|`로 이스케이프합니다

명령이 실행되기 직전에 다음 두 개의 특수 플레이스홀더가 치환됩니다:

- `%fm_sender%` -> FM Data를 보낸 플레이어
- `%fm_data%` -> 클라이언트로부터 수신한 데이터 값

명령은 서버 명령으로 실행됩니다.

## 예시 명령

어떤 플레이어의 버튼 클릭에 반응하기:

```mcfunction
/fmdata listener add button_ping equals equals false false @a ui.button pressed "tellraw @a {\"text\":\"%fm_sender%가 버튼을 눌렀습니다\"}"
```

데이터에 `gold`가 포함되면 여러 명령 실행하기:

```mcfunction
/fmdata listener add reward equals contains false true @a reward "gold" "say %fm_sender%로부터 보상: %fm_data%|||effect give %fm_sender% minecraft:speed 3 1 true"
```

# 웰컴 데이터

웰컴 데이터는 플레이어가 접속할 때 일치하는 플레이어에게 FM Data를 전송합니다.
...
- `<target_player>`는 `@a`, `@p`, `@s` 같은 일반 셀렉터를 지원합니다
- 데이터는 접속한 일치 플레이어에게 전송됩니다
- 항목은 자동으로 저장되고 로드됩니다

## 예시 명령

접속하는 모든 플레이어에게 웰컴 데이터 보내기:

```mcfunction
/fmdata welcome_data add welcome_all @a hud.welcome "환영합니다!"
```

한 명의 플레이어에게만 웰컴 데이터 보내기:

```mcfunction
/fmdata welcome_data add welcome_vip Player761 hud.vip "VIP 혜택이 활성화되었습니다"
```
...
3. 먼저 간단하게 시작하세요: 복잡한 리스너를 만들기 전에 `/fmdata send`로 테스트하세요.
