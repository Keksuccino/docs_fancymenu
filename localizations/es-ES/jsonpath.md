---
title: Rutas JSON
description: Cómo usar Jayway JsonPath en el marcador de posición JSON Parser.
---
...
un documento XML. El "objeto miembro raíz" en JsonPath siempre se denomina `$`, tanto si es un objeto como un array.
...
| :------------------------ | :----------------------------------------------------------------- |
| `$`                       | El elemento raíz que consultar. Esto inicia todas las expresiones de ruta.       |
...
| `[?(<expression>)]`       | Expresión de filtro. La expresión debe evaluarse a un valor booleano.    |


## Funciones

Las funciones pueden invocarse al final de una ruta: la entrada de una función es el resultado de la expresión de ruta.
La salida de la función la determina la propia función.

| Función                  | Descripción                                                         | Tipo de salida |
| :------------------------ | :------------------------------------------------------------------ |:----------- |
| min()                     | Devuelve el valor mínimo de un array de números                       | Double      |
| max()                     | Devuelve el valor máximo de un array de números                       | Double      |
| avg()                     | Devuelve el valor medio de un array de números                   | Double      | 
| stddev()                  | Devuelve el valor de la desviación estándar de un array de números        | Double      | 
| length()                  | Devuelve la longitud de un array                                     | Integer     |
| sum()                     | Devuelve la suma de un array de números                       | Double      |
| keys()                    | Devuelve las claves de las propiedades (una alternativa al tilde final `~`)  | `Set<E>`    |
| concat(X)                 | Devuelve una versión concatenada de la salida de la ruta con un nuevo elemento  | como la entrada  |
| append(X)                 | añade un elemento al array de salida de json path                           | como la entrada  |

## Operadores de filtro

Los filtros son expresiones lógicas utilizadas para filtrar arrays. Un filtro típico sería `[?(@.age > 18)]`, donde `@` representa el elemento actual que se está procesando. Se pueden crear filtros más complejos con los operadores lógicos `&&` y `||`. Los literales de cadena deben ir entre comillas simples o dobles (`[?(@.color == 'blue')]` o `[?(@.color == "blue")]`).   
...
| =~                       | la parte izquierda coincide con una expresión regular  [?(@.name =~ /foo.*?/i)]             |
...
| JsonPath (haz clic en el enlace para probarlo)| Resultado |
| :------- | :----- |
...
| <a href="http://jsonpath.herokuapp.com/?path=$..book[2:]" target="_blank">$..book[2:]</a>                | Libro número dos contando desde el final          |
