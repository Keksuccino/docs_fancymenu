...
| `scale` | No | Multiplica los resultados numéricos de `value`; valor predeterminado `1.0` |
| `return_type` | No | `value`, `string`, `snbt` o `json`; valor predeterminado `value` |

Las posiciones de bloque del lado del cliente no admiten coordenadas `~` ni `^`.
...
`@e` no selecciona la entidad más cercana a menos que añadas `sort=nearest`. También se admite la búsqueda directa por UUID y por nombre exacto de entidad.

Opciones de selector admitidas:

| Opción | Descripción |
|---|---|
| `type` | ID de entidad; antepón `!` para excluir |
| `name` | Nombre exacto para mostrar; antepón `!` para excluir |
| `tag` | Etiqueta de entidad; antepón `!` para excluir |
...
| Tipo | Resultado |
|---|---|
| `value` | Las etiquetas numéricas se formatean numéricamente y aplican `scale`; las etiquetas de texto devuelven su texto; otras etiquetas devuelven texto con aspecto SNBT |
| `string` | Devuelve el valor de cadena de la etiqueta, o una cadena vacía cuando la etiqueta no tiene valor de cadena |
| `snbt` | Devuelve la representación SNBT de la etiqueta |
| `json` | Solo para etiquetas compuestas: devuelve un componente de texto de Minecraft serializado que contiene una salida NBT detallada |
...
- Selectores de entidades completos del lado del servidor.
- Objetivos de bloques con coordenadas absolutas, relativas (`~`) o locales (`^`).
...
El marcador de posición devuelve un valor vacío hasta que llega la respuesta del servidor. Las respuestas se almacenan en caché brevemente para evitar solicitudes excesivas.
