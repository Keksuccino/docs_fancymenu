...
Eine der stärksten Funktionen des Platzhaltersystems von FancyMenu ist die Möglichkeit, Platzhalter in andere Platzhalter einzubetten. Das bedeutet, dass du die Ausgabe eines Platzhalters als Eingabe für einen anderen verwenden kannst.
...
Die meisten Elemente mit Texteingaben unterstützen Platzhalter. Du kannst beim Bearbeiten sehen, ob ein Texteingabefeld Platzhalter unterstützt. Wenn sich beim Bearbeiten des Textes der **Vollbild-Texteditor** öffnet, werden Platzhalter unterstützt.
...
Oben in der Platzhalterliste befindet sich eine **Suchleiste**, mit der du Platzhalter durchsuchen kannst.

Wenn du in der Platzhalterliste auf einen Platzhalter klickst, wird er in den Textinhalt eingefügt.

# Platzhalter im Detail

Diese Liste enthält die meisten, wenn nicht sogar alle in FancyMenu verfügbaren Platzhalter. Die Liste kann durch Mod-Updates gelegentlich etwas veraltet sein.
...
## Version des Mod-Loaders (loaderver)
Gibt die Version des Mod-Loaders zurück (Forge/Fabric).
```
{"placeholder":"loaderver"}
```
Beispielausgabe: `43.2.0`

## Name des Mod-Loaders (loadername)
...
Gibt die Anzahl der derzeit geladenen Mods zurück.
```
...
## Wert einer Minecraft-Option (minecraft_option_value)
...
```
Parameter:
- `type`: Legt fest, welche Art von Informationen zurückgegeben werden
  - `"both"`: Gibt die zuletzt geöffnete Welt oder den zuletzt verwendeten Server zurück (Standard)
  - `"server"`: Gibt nur dann etwas zurück, wenn der zuletzt verwendete Eintrag ein Server war
  - `"world"`: Gibt nur dann etwas zurück, wenn der zuletzt verwendete Eintrag eine Welt war
- `full_world_path`: Steuert, wie Weltpfade angezeigt werden
  - `"true"`: Gibt den vollständigen Weltpfad zurück (Standard)
  - `"false"`: Gibt nur den Weltnamen ohne Pfad zurück (wirkt sich nicht auf Server aus)
...
## Aktuelle Bildschirmkennung (screenid)
...
Gibt den aktuellen Wert eines benutzerdefinierten oder Vanilla-Texteingabefelds anhand der Elementkennung zurück.
```
{"placeholder":"text_input_field_value","values":{"element_identifier":"my_input"}}
```
Beispielausgabe: `Hello World`

## Aktuelle Spieler-Gesundheit (current_player_health)
Gibt die aktuellen Gesundheitspunkte des Spielers zurück.
```
{"placeholder":"current_player_health"}
```
Beispielausgabe: `20.0`

## Maximale Spieler-Gesundheit (max_player_health)
...
## Aktuelle Spieler-Gesundheit (Prozent) (current_player_health_percent)
Gibt die Gesundheit des Spielers als Prozentwert zurück.
```
...
Gibt die Absorptionsgesundheit des Spielers zurück (goldene Herzen).
```
...
Gibt die Absorptionsgesundheit des Spielers als Prozentwert zurück.
```
...
Gibt den Hunger des Spielers als Prozentwert zurück.
```
...
Gibt den gesamten Rüstungshärte-Wert des Spielers zurück.
```
...
Gibt die Rüstung des Spielers als Prozentwert zurück.
```
...
Gibt den Sauerstoffwert des Spielers als Prozentwert zurück.
```
{"placeholder":"current_player_oxygen_percent"}
```
Beispielausgabe: `100`

## Aktuelle Spieler-Stufe (current_player_level)
...
## Aktuelle Spieler-Erfahrung (current_player_exp)
...
Gibt den Fortschritt des Spielers zur nächsten Stufe als Prozentwert zurück.
```
...
Gibt den Angriffscooldown des Spielers als Prozentwert zurück.
```
...
Gibt die Richtung zurück, in die der Spieler schaut.
```
...
Gibt die aktuelle Gesundheit der Entität zurück, auf der der Spieler reitet.
```
...
Gibt die maximale Gesundheit der Entität zurück, auf der der Spieler reitet.
```
...
Gibt die Gesundheit des Reittiers als Prozentwert zurück.
```
...
Gibt den Wert des Sprungkraftbalkens des Reittiers zurück.
```
...
Gibt den derzeit ausgewählten Hotbar-Slot zurück (0-8).
```
...
Gibt die Stapelgröße des Gegenstands in einem bestimmten Inventarslot des Spielers zurück.
```
...
Gibt Haltbarkeitsinformationen für den Gegenstand in einem bestimmten Inventarslot des Spielers zurück.
```
{"placeholder":"slot_item_durability","values":{"slot":"0","format":"percentage"}}
```
Parameter:
- `slot`: Nummer des Inventarslots des Spielers.
- `format`: `current`, `remaining`, `max`, `damage`, `percentage` oder `percent`.

Beispielausgabe: `87`

## Anzeigename des Slot-Gegenstands (slot_item_display_name_fm)
Gibt den Anzeigenamen des Gegenstands in einem bestimmten Slot als JSON-Textkomponente zurück. Im Zuschauermodus können Hotbar-Slots zu Namen von Zuschauermenü-Elementen aufgelöst werden, sofern `ignore_spectator` nicht `true` ist.
```
{"placeholder":"slot_item_display_name_fm","values":{"slot":"0","ignore_spectator":"false"}}
```
Beispielausgabe: `{"text":"Diamond Sword","color":"aqua"}`

## Anzahl eines Inventargegenstands (inventory_item_count)
...
## Sättigungswert eines Inventarslots für Lebensmittel (inventory_slot_food_point_restore_amount)
Gibt die Hungerpunkte zurück, die der Lebensmittelgegenstand im angegebenen Inventarslot des Spielers wiederherstellt.
```
{"placeholder":"inventory_slot_food_point_restore_amount","values":{"slot":"0"}}
```
Beispielausgabe: `4.0`

## Überwachsener Inventargegenstand (hovered_inventory_item)
Gibt den Gegenstands-Schlüssel des derzeit in einem Inventarbildschirm überfahrenen Gegenstands zurück.
```
...
## Stunde der Tageszeit der Welt (world_daytime_hour)
Gibt die Stundenkomponente der Weltzeit zurück. Standardmäßig wird das 24-Stunden-Format verwendet; setze `twelve_hour_format` auf `"true"`, um das 12-Stunden-Format zu verwenden.
```
{"placeholder":"world_daytime_hour","values":{"twelve_hour_format":"false"}}
```
Beispielausgabe: `12`

## Minute der Tageszeit der Welt (world_daytime_minute)
...
## Schwierigkeitsgrad der Welt (world_difficulty)
...
Gibt das Biom zurück, in dem sich der Spieler derzeit befindet. Setze `as_key` auf `"false"`, um sofern verfügbar einen übersetzten/angezeigten Namen zurückzugeben.
```
...
Gibt die Dimension zurück, in der sich der Spieler derzeit befindet. Setze `as_key` auf `"false"`, um sofern verfügbar einen übersetzten/angezeigten Namen zurückzugeben.
```
...
Gibt den aktuellen Wert einer Gamerule in der geladenen Welt/dem geladenen Server zurück. Serverwelten erfordern FancyMenu auf dem Server.
```
...
Gibt die Kreativtab-Kategorie eines Gegenstands zurück. Setze `as_key` auf `"true"`, um statt des Anzeigenamens den Kategorie-Schlüssel zurückzugeben.
```
...
Gibt die aktuelle Vanilla-Actionbar-Nachricht über der Hotbar zurück.
```
...
Gibt an, wie viele Ticks die aktuelle Vanilla-Actionbar-Nachricht noch angezeigt wird.
```
{"placeholder":"action_bar_message_time_fm"}
```
Beispielausgabe: `42`

## Kameradrehung X (camera_rotation_x_fm)
...
## Kameradrehung Y (camera_rotation_y_fm)
Gibt die aktuelle Kameradrehung in Grad zurück.
```
{"placeholder":"camera_rotation_y_fm"}
```
Beispielausgabe: `-90.0`

## Kameradrehung Delta X (camera_rotation_delta_x_fm)
Gibt die pro Tick auftretende Änderung der Kameraneigung zurück.
```
{"placeholder":"camera_rotation_delta_x_fm"}
```
Beispielausgabe: `0.4`

## Kameradrehung Delta Y (camera_rotation_delta_y_fm)
Gibt die pro Tick auftretende Änderung der Kameradrehung zurück.
```
...
## Fortschritt der Gegenstandsnutzung des Spielers (player_item_use_progress_fm)
...
Gibt die pro Tick auftretende Änderung der Spielerposition auf der X-Achse zurück.
```
...
Gibt die pro Tick auftretende Änderung der Spielerposition auf der Y-Achse zurück.
```
...
Gibt die pro Tick auftretende Änderung der Spielerposition auf der Z-Achse zurück.
```
...
Gibt eine Liste aller Spieler zurück, die sich derzeit in der Welt befinden.
```
...
## Spielerzahl des Servers (serverplayercount)
...
Gibt die CPU-Auslastung der JVM als Prozentwert zurück.
```
{"placeholder":"jvmcpu"}
```
...
Gibt die CPU-Auslastung des Betriebssystems als Prozentwert zurück.
```
{"placeholder":"oscpu"}
```
...
Gibt die aktuelle Bildrate zurück.
```
{"placeholder":"fps"}
```
Beispielausgabe: `120`
...
Gibt die aktuell verwendete RAM-Menge (MB) zurück.
```
{"placeholder":"usedram"}
```
...
Gibt den maximal zugewiesenen RAM (MB) zurück.
```
{"placeholder":"maxram"}
```
...
Gibt den Tracknamen eines Audio-Elements zurück.
```
...
```
Beispielausgabe: `01:30` (oder `45`, wenn `show_percentage` auf `"true"` gesetzt ist)

## Wiedergabestatus von Audio (audio_playing_state)
...
Gibt die Lautstärke eines Video-Elements zurück (0.0 bis 1.0).
```
...
Gibt die Gesamtdauer eines Video-Elements im Format `MM:SS` zurück. Setze `output_as_timestamp` auf `"true"`, um einen Zeitstempel in Millisekunden zurückzugeben.
```
{"placeholder":"video_element_duration","values":{"element_identifier":"my_video_element","output_as_timestamp":"false"}}
```
Beispielausgabe: `02:00` (oder `120000`, wenn `output_as_timestamp` auf `"true"` gesetzt ist)

## Wiedergabezeit eines Video-Elements (video_element_playtime)
Gibt die aktuelle Wiedergabezeit (Fortschritt) eines Video-Elements im Format `MM:SS` zurück. Setze `show_percentage` auf `"true"` für einen Fortschrittswert von 0-100 oder `output_as_timestamp` auf `"true"` für Millisekunden.
```
...
Gibt die Lautstärke eines Video-Menühintergrunds zurück (0.0 bis 1.0).
```
...
Gibt die Gesamtdauer eines Video-Menühintergrunds im Format `MM:SS` zurück. Setze `output_as_timestamp` auf `"true"`, um einen Zeitstempel in Millisekunden zurückzugeben.
```
{"placeholder":"video_background_duration","values":{"background_identifier":"main_menu_video","output_as_timestamp":"false"}}
```
Beispielausgabe: `03:00` (oder `180000`, wenn `output_as_timestamp` auf `"true"` gesetzt ist)

## Wiedergabezeit eines Video-Hintergrunds (video_background_playtime)
Gibt die aktuelle Wiedergabezeit (Fortschritt) eines Video-Menühintergrunds im Format `MM:SS` zurück. Setze `show_percentage` auf `"true"` für einen Fortschrittswert von 0-100 oder `output_as_timestamp` auf `"true"` für Millisekunden.
```
...
## Rechner (calc)
Der Rechner-Platzhalter ist ein leistungsstarkes Werkzeug, mit dem du mathematische Berechnungen direkt in deinen Layouts durchführen kannst. Er unterstützt eine breite Palette mathematischer Operationen und funktioniert sowohl mit Dezimal- als auch mit Ganzzahlen.
...
- `decimal`: Legt fest, ob das Ergebnis Dezimalstellen enthalten soll (`true`) oder auf Ganzzahlen gerundet wird (`false`)
...
Rundet eine Zahl. Standardmäßig wird auf die nächstgelegene ganze Zahl gerundet; setze `decimals` auf eine nicht-negative Zahl, um auf diese Anzahl von Dezimalstellen zu runden.
```
{"placeholder":"math_round","values":{"num":"3.14159","decimals":"2"}}
```
Beispielausgabe: `3.14` (bei `decimals:-1` oder wenn weggelassen → `3`)
...
## Stringify (stringify)
Macht Text zu einer Zeichenkette, indem alle Syntaxzeichen maskiert werden.
```
...
```
Beispielausgabe: Textinhalt aus der URL
...
```
Parameter:
- `source`: Die Quelle der Textzeilen (ersetzt den alten Parameter `path`)
...
## JSON-Parser (json)
Analysiert JSON-Daten aus einer Datei, einer URL oder direkt aus JSON-Inhalt und extrahiert Werte mithilfe von JSON-Pfad-Ausdrücken.
```
...
## Zeichenanzahl von Text (text_character_count)
...
Wandelt den Eingabetext vollständig in Großbuchstaben um.
```
...
Wandelt den Eingabetext vollständig in Kleinbuchstaben um.
```
{"placeholder":"lowercase_text","values":{"text":"Hello World"}}
```
Beispielausgabe: `hello world`

## Text in Titelkasus (title_case_text)
Wandelt den Eingabetext in Titelkasus um.
```
{"placeholder":"title_case_text","values":{"text":"hello world"}}
```
Beispielausgabe: `Hello World`

## Text in Satzschreibung (sentence_case_text)
Wandelt den Eingabetext in Satzschreibung um.
```
...
Wandelt den Eingabetext in wechselnde Groß-/Kleinschreibung um.
```
{"placeholder":"alternating_case_text","values":{"text":"alternating case"}}
```
Beispielausgabe: `aLtErNaTiNg CaSe`

## Groß-/Kleinschreibung umschalten (toggle_case_text)
Schaltet die Groß-/Kleinschreibung jedes Buchstabens im Eingabetext um.
```
...
Dekodiert eine Base64-Zeichenkette zurück in Klartext.
```
...
```
Parameter:
- `path_or_url`: Dateipfad oder URL, aus der gelesen werden soll
- `mode`: Entweder `"all"` (gibt alle Zeilen zurück) oder `"last"` (gibt nur die letzten X Zeilen zurück)
- `separator`: Text zum Verbinden der Zeilen (Standard: `"\n"`)
- `last_lines`: Anzahl der Zeilen, die bei `mode` `"last"` zurückgegeben werden sollen (Standard: `"1"`)
...
```
Beispielausgabe: Der Text, der sich gerade in der Zwischenablage befindet
...
- `use_regex`: Ob Regex verwendet werden soll (`"true"`) oder ein wörtlicher Vergleich (`"false"`)
- `replace_all`: Alle Vorkommen ersetzen (`"true"`) oder nur das erste (`"false"`)

Beispielausgabe: `Hello FancyMenu! This is a test.`

## Fallunterscheidung (switch_case)
Führt basierend auf einem Wert eine Switch-Case-Operation aus.
```
...
Ruft NBT-Daten auf dem Client ab (ähnlich dem Befehl `/data get`). Verwende die Server-Variante `nbt_data_get_server`, wenn du mit einem Server verbunden bist und verlässliche serverseitige Werte benötigst.
```
...
- `entity_selector`: Entitätselektor wie `@s`, `@p`, `@e` oder UUID/Name (für Entitäten)
- `block_pos`: Blockposition im Format `"x y z"` (für Blöcke)
- `nbt_path`: Der abzurufende NBT-Pfad
- `scale`: Optionale Skalierung für numerische Werte (Standard: `"1.0"`)
- `return_type`: Wie die Daten zurückgegeben werden sollen:
  - `"value"`: Standard, gibt den Wert zurück (mit optionaler Skalierung für Zahlen)
  - `"string"`: Gibt die tatsächlichen NBT-Daten als Zeichenkette zurück
...
Fragt NBT-Daten auf der Serverseite ab (über ein Paket) und speichert die Ergebnisse kurzzeitig im Cache. Die Werte entsprechen dem clientseitigen Platzhalter.
```
...
Gibt die zuletzt aufgezeichnete Todesnachricht des Client-Spielers zurück. Setze `as_json_component` auf `"true"`, um die rohe JSON-Textkomponente zu erhalten.
```
...
Gibt zurück, wie lange FancyMenu geladen ist. Standardmäßig ist der Wert in Sekunden; setze `output_as_millis` auf `"true"`, um Millisekunden zu erhalten.
```
{"placeholder":"uptime_duration","values":{"output_as_millis":"false"}}
```
Beispielausgabe: `742` (Sekunden seit dem Laden)

## Weltspeichernamen (level_save_names)
Listet alle lokalen Weltspeichernamen auf, verbunden durch das gewählte Trennzeichen. Wird im Client-Thread ausgeführt.
```
{"placeholder":"level_save_names","values":{"separator":", "}}
```
Beispielausgabe: `Creative Test, Survival World, Hardcore`

## Weltspeicherdaten (level_save_data)
Gibt serialisierte Level-Daten für den angegebenen Weltnamen zurück (muss dem Anzeigenamen entsprechen, der in der Speicherliste angezeigt wird).
```
{"placeholder":"level_save_data","values":{"level_name":"Survival World"}}
```
Beispielausgabe: `{"name":"Survival World","gameMode":"survival",...}`

## Zahlenbasis-Konverter (number_base_convert)
Konvertiert eine Zahl (Ganzzahl oder Bruchzahl) von einer Basis in eine andere (2–36). Standardmäßig wird Dezimal verwendet, wenn keine Basen angegeben werden.
```
...
## Datei-MD5 (file_md5)
Gibt den MD5-Hash einer lokalen Datei als Kleinbuchstaben-Hexzeichenkette zurück.
```
...
## Eine dynamische Speicheranzeige erstellen
```
...
1. **Teure Operationen cachen**: Einige Platzhalter (z. B. solche, die Systeminformationen lesen) können ressourcenintensiv sein. Ziehe in Betracht, ihre Werte in Variablen zu speichern, wenn du sie mehrfach verwenden musst.

2. **Geeignete Dezimaleinstellungen verwenden**: Arbeite bei Berechnungen passend mit dem Parameter `decimal`. Setze ihn auf `false`, wenn du ganze Zahlen brauchst, und auf `true`, wenn du präzise Dezimalwerte benötigst.

3. **Fehlende Werte behandeln**: Berücksichtige immer, was passieren soll, wenn ein Platzhalter keinen Wert zurückgibt. In solchen Fällen möchtest du möglicherweise Standardwerte angeben.

4. **Leistung testen**: Wenn du viele Platzhalter oder komplexe verschachtelte Strukturen verwendest, teste die Auswirkungen auf die Leistung, besonders auf schwächeren Systemen.

5. **Erweiterte Größen-/Positionsanpassung verwenden**: Kombiniere für dynamische UI-Elemente Platzhalter mit erweiterter Größen- und Positionsanpassung, um responsive Layouts zu erstellen.
...
- Ob du bei der Platzhalter-ID die richtige Groß-/Kleinschreibung verwendest
- Ob der Platzhalter bestimmte Bedingungen benötigt, um aktualisiert zu werden

## Verschachtelte Platzhalter funktionieren nicht
Beim Verschachteln von Platzhaltern:
- Stelle sicher, dass Anführungszeichen korrekt maskiert sind
- Vergewissere dich, dass jeder verschachtelte Platzhalter für sich allein gültig ist

## Leistungsprobleme
Wenn du Leistungsprobleme bemerkst:
- Reduziere die Anzahl der verwendeten Platzhalter
- Vermeide unnötige Verschachtelungen
- Ziehe in Betracht, häufig verwendete Werte in Variablen zu speichern
- Verwende den passenden Platzhalter für deinen Bedarf (z. B. keine Realtime-Platzhalter, wenn statische Werte ausreichen)
