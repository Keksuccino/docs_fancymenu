---
title: Chemins JSON
description: Comment utiliser Jayway JsonPath dans le placeholder JSON Parser.
---

# Chemins JSON dans FancyMenu

Le placeholder **JSON Parser** de FancyMenu utilise [Jayway JsonPath](https://github.com/json-path/JsonPath) pour récupérer du contenu à partir de fichiers JSON.
...
Les expressions JsonPath font toujours référence à une structure JSON de la même manière que les expressions XPath sont utilisées avec 
un document XML. Le « root member object » dans JsonPath est toujours désigné par `$`, qu’il s’agisse d’un objet ou d’un tableau.
...
| `..`                      | Parcours en profondeur. Disponible partout où un nom est requis.   |
| `.<name>`                 | Enfant en notation pointée                                         |
| `['<name>' (, '<name>')]` | Enfant ou enfants en notation par crochets                         |
| `[<number> (, <number>)]` | Index ou indices de tableau                                        |
| `[start:end]`             | Opérateur de tranche de tableau                                    |
| `[?(<expression>)]`       | Expression de filtre. L’expression doit être évaluée à une valeur booléenne. |


## Fonctions

Les fonctions peuvent être appelées à la fin d’un chemin - l’entrée d’une fonction est la sortie de l’expression de chemin.
La sortie de la fonction est déterminée par la fonction elle-même.
...
| stddev()                  | Fournit la valeur de l’écart type d’un tableau de nombres          | Double      | 
| length()                  | Fournit la longueur d’un tableau                                    | Integer     |
| sum()                     | Fournit la somme d’un tableau de nombres                             | Double      | 
...
Les filtres sont des expressions logiques utilisées pour filtrer des tableaux. Un filtre classique serait `[?(@.age > 18)]`, où `@` représente l’élément courant en cours de traitement. Des filtres plus complexes peuvent être créés avec les opérateurs logiques `&&` et `||`. Les littéraux de chaîne doivent être entourés de guillemets simples ou doubles (`[?(@.color == 'blue')]` ou `[?(@.color == "blue")]`).   

| Operator                 | Description                                                           |
| :----------------------- | :-------------------------------------------------------------------- |
| ==                       | la valeur de gauche est égale à celle de droite (notez que 1 n’est pas égal à '1')              |
| !=                       | la valeur de gauche est différente de celle de droite                                            |
...
| size                     | la taille de la valeur de gauche (tableau ou chaîne) doit correspondre à celle de droite                     |
...
| <a href="http://jsonpath.herokuapp.com/?path=$.store.*" target="_blank">$.store.*</a>                  | Toutes les entrées, livres et vélos compris  |
...
| <a href="http://jsonpath.herokuapp.com/?path=$..book[:2]" target="_blank">$..book[:2]</a>                | Tous les livres de l’index 0 (inclus) jusqu’à l’index 2 (exclu) |
| <a href="http://jsonpath.herokuapp.com/?path=$..book[1:2]" target="_blank">$..book[1:2]</a>                | Tous les livres de l’index 1 (inclus) jusqu’à l’index 2 (exclu) |
...
| <a href="http://jsonpath.herokuapp.com/?path=$.store.book[?(@.price < 10)]" target="_blank">$.store.book[?(@.price < 10)]</a> | Tous les livres du magasin coûtant moins de 10  |
...
| <a href="http://jsonpath.herokuapp.com/?path=$..*" target="_blank">$..*</a>                        | Montre-moi tout   
