...
FancyMenu inyecta un puente de JavaScript en cada función respaldada por MCEF (por ejemplo, el elemento **Browser**). El puente permite que el contenido web:
...
Usa el evento `fancymenu-ready`, o detección de disponibilidad, para asegurarte de que el puente esté disponible antes de llamarlo.
...
El contenido también puede alojarse localmente: coloca archivos HTML en `config/fancymenu/assets/` y cárgalos mediante URLs con el formato `file:///config/fancymenu/assets/<nombre>.html`.
...
Los ayudantes heredados `fancymenu.execute(...)` y `fancymenu.executeWithCallback(...)` siguen funcionando y delegan al espacio de nombres `actions`, así que no es necesario hacer cambios inmediatos en el contenido existente.
```
...
- `opengui` – abre un menú de FancyMenu o una pantalla de vanilla (valor: identificador de pantalla)
- `openlink` – abre un navegador (valor: URL)
- `sendmessage` – publica un mensaje en el chat (valor: texto del mensaje)
- `set_variable` – asigna una variable de FancyMenu (valor: `nombre:valor`
- `joinserver` – conecta a un servidor (valor: dirección)
- `disconnect_server_or_world` – desconecta y va a una pantalla de destino (valor: identificador de pantalla)

Toda acción que exista en FancyMenu está disponible a través del puente; consulta los [scripts de acción](./action-scripts) para ver el catálogo completo.

## 4. Lectura de placeholders

El sistema de [placeholder](/placeholders) de FancyMenu se expone mediante `fancymenu.placeholders` (y `FancyMenu.placeholders`). Ambos métodos auxiliares regresan `Promise<string>`:
    ...
    <script>
        ...
        function loadPlaceholders() {
            ...
            Promise.all([
                ...
                console.error('La solicitud del placeholder falló:', error);
            });
        }
    </script>
</body>
...
- **Seguridad**: las acciones se ejecutan con los permisos normales del jugador. Trata con cuidado los datos proporcionados por el usuario para evitar inyección.
...
4. Revisa el registro de Minecraft (`latest.log`) para ver mensajes de error de FancyMenu si la ejecución falla de manera inesperada.
