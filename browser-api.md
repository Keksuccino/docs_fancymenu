...
```javascript
fancymenu.execute('quitgame');                // action without value
fancymenu.execute('opengui', 'title_screen'); // action with value
fancymenu.execute('set_variable', 'hp:20');   // value uses name:value format
```

### With Callbacks

```javascript
fancymenu.executeWithCallback(
    'opengui',
    'title_screen',
