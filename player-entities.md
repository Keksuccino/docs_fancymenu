...
In Minecraft 1.21.1+ you can simply use the normal resize grabbers of the element to scale the entity.

For older versions (1.21.0 and older), Player Entity elements do not support direct resizing via the resize grabbers. Instead you need to right-click the element and click on **Scale**. This allows you to set a scale for the element. The default one should be `30`, so setting it to `60` for example makes the player twice as big as normal, setting it to `15` shows it at half the size and so on.

# Dependency: Fancy Entity Renderer (FER)

For Minecraft 1.21.1+, there is an extra mod needed for making Player Entity elements work. The mod is called "Fancy Entity Renderer" and is available on CurseForge and Modrinth.
